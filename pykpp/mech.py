...
from .stdfuncs import solar_noon_local, solar_noon_utc, solar_declination
...
def addtojac(rxni, reaction, jac, allspcs):
    ...
    spcorder = [
        (
            'y[%d]**(%s)' % (allspcs.index(spc_), stc_)
        ).replace('**(1.)', '').replace('**(+ 1.)', '')
        for stc_, spc_ in reaction['reactants']
    ]

    # Iterate over all stoichiometries and spcs names for
    # reactions and products in reaction
    for role, stc, spc in (
        [('r', stc_, spc_) for stc_, spc_ in reaction['reactants']]
        + [('p', stc_, spc_) for stc_, spc_ in reaction['products']]
    ):
        ...
        for rstc, rspc in reaction['reactants']:
            ...
            if expr[1:] in jac[ispc][irspc] and expr not in jac[ispc][irspc]:
                ...
                jac[ispc][irspc] = jac[ispc][irspc].replace(
                    sign + expr[1:], ''
                )
            else:
...
class Mech(object):
    ...
    def add_world_updater(self, updater):
        """
        Add callable updater to list of updaters. See Update_World for more
        details.

        Arguments
        ---------
        updater : updaters.updater
            Must support updatenow method and be callable.
    ...
    def add_func_updater(self, *args, **keywords):
        """
        Add a function updater using func_updater from stdfuncs
        
        Arguments
        ---------
        args : iterable
            passed to stdfunc.fun_updater
        keywords : mappable
            passed to stdfunc.fun_updater
        ...
        None
        """
        self.add_world_updater(stdfuncs.func_updater(*args, **keywords))
    ...
    def __init__(
        ...
        mechname : str
            name for output
        keywords : mappable
            ignore certain keywords from reactants in calculate reaction rates
            If None, defaults to ['hv', 'PROD', 'EMISSION']
        timeunit : str
            'local' or 'utc'
        add_default_funcs : bool
            if True, then add updaters based on rate constants  having:
            - M : Update_M
            - THETA : Update_THETA
            - SUN : Update_SUN
        incr : int
            default functions will be called when simulated
            time has progressed more than incr seconds
        monitor_incr : int
            If monitor_incr is not None, add a monitor function
            updaters, so it has a special optional keyword
        doirr : bool
            save reaction rates for further analysis
        verbose : int
            add printing
        ...
        nocfactorkeys = (
            tuple(world.keys())
            + ('CFACTOR', 'TEMP', 'P', 'StartDate', 'StartJday',
               'Latitude_Degrees', 'Latitude_Radians', 'Longitude_Degrees',
               'Longitude_Radians')
        )
        ...
        if 'MONITOR' in self._parsed.keys():
            ...
            self.monitor_expr = tuple([
                (self.allspcs.index(k_) if k_ in self.allspcs else None, k_)
                for k_ in monitor])
        else:
    ...
    def set_incr(self, incr=None):
        ...
        ---------
        incr : int
            time increment (in rate unit); if None, select minimum time from
        ...
        if incr is None:
            self.incr = min([
                func.incr for func in self.updaters + self.constraints])
        else:
            self.incr = incr
    ...
    def add_funcs(self):
        ...
        if not self.add_default_funcs:
            return
        add_func = [
            (usetheta, stdfuncs.Update_THETA),
            (usesun, stdfuncs.Update_SUN),
            (usem, stdfuncs.Update_M)
        ]
        for check, func in add_func:
            if check and func not in self.updaters:
                self.add_world_updater(stdfuncs.func_updater(
                    func, incr=self.incr, verbose=self.verbose
                ))
    ...
    def set_rate_exp(self):
        ...
        3) Define drate_exp, fill_drate_exp (and _str's)
           ...
           fill_drate_exp - can be executed to fill the drate_per_dspc array,
                       ...
                       but is much faster because it only allocates the memory
                       once.
        ...
        for rxni, reaction in enumerate(self._parsed['EQUATIONS']):
            ...
            spcorder = [
                (
                    'y[%d]**(%s)' % (self.allspcs.index(spc_), stc_)
                ).replace('**(1.)', '').replace('**(1.)', '')
                for stc_, spc_ in reaction['reactants']]
        ...
        drate_exp = [
            ['0' if col == '' else col for col in row] for row in drate_exp]
        self.drate_exp_str = (
            'array([' + ','.join([
                '[' + ','.join(row) + ']' for row in drate_exp
            ]) + '])')
        self.drate_per_dspc = np.zeros((nspcs, nspcs), dtype='d')
        self.fill_drate_exp_str = '\n'.join([
            '\n'.join([
                'drate_per_dspc[%i,%i] = %s' % (j, i, v)
                for i, v in enumerate(row) if v != '0'
            ])
            for j, row in enumerate(drate_exp)
        ])
        self.fill_drate_exp = compile(
            self.fill_drate_exp_str, 'drate_exp', 'exec')
        self.drate_exp = compile(self.drate_exp_str, 'drate_exp', 'eval')
        self.rate_const_exp_str = (
            'array([' + ',\n '.join(rate_const_exp) + '])')
        self.rate_const_exp = compile(
            self.rate_const_exp_str, 'rate_const_exp', 'eval')
        self.fill_rate_const_exp = compile(
            self.fill_rate_const_exp_str + 'rate_const[:] = '
            + self.rate_const_exp_str, 'fill_rate_const_exp', 'exec')
        self.rate_const = np.zeros((nrxns,), dtype='d')
        self.rate_exp_str = 'array([' + ',\n '.join(rate_exp) + '])'
        self.fill_rate_exp_str = '\n'.join([
            'rates[%d] = %s' % ri_rassign for ri_rassign in enumerate(rate_exp)
        ])
        self.fill_rate_exp = compile(
            self.fill_rate_exp_str, 'fill_rate_exp', 'exec')
        self.rate_exp = compile(self.rate_exp_str, 'rate_exp', 'eval')

    def set_dy_exp(self):
        """
        Set the dy property based on the parsed EQUATIONS
        ...
        for si, spc in enumerate(self.allspcs):
            for ri, stoic in dy_stoic[spc].items():
                dy_exp[si] += (
                    ' + %f * rates[%d]' % (stoic, ri)
                ).replace('+ -1.000000 * ', '-').replace('+ 1.000000 * ', '+')

        dy_exp = ['0' if dy_ == '' else dy_ for dy_ in dy_exp]
        self.dy_exp_str = 'array([' + ',\n '.join(dy_exp) + '])'
        self.fill_dy_exp_str = '\n'.join([
            'dy[%d] = %s' % yi_assign for yi_assign in enumerate(dy_exp)
        ])
    ...
    def set_mechname(self, mechname):
        ...
        if mechname is None:
            ...
            if isinstance(self.path, (str,)):
                ...
                self.mechname, dummy = os.path.splitext(
    ...
    def Update_World(self, world=None, forceupdate=False):
        """
        Update_World is designed to watch time (t) and make updates to world as
        appropriate. The updates are based on the elapsed time since last
        updated compared to specified time increment thresholds (incr).

        This has two main steps:

        1. Check each updater in self.updaters. If time elapsed since last run
           is greater than updater's incr property, it will be called.
        2. If any updater was called or if the time since Mech's last_updated
           property is greater than the Mech's incr property, then update rate
           coefficients by calling:
           - stdfuncs.update_func_world(self, world) to prepare environment
           - self.update_rate_const() to update rate coefficients.

        Arguments
        ---------
        world : mappable
            dictionary defining current state
        forceupdate : bool
            boolean that forces all functions to update
    ...
    def resetworld(self):
        ...
        to control solar properties
            ...
            Latitude_Degrees = 45.
            # if Latitude_Radians is provided: degrees(Latitude_Radians)
            Latitude_Radians = radians(Latitude_Degrees)
            SunRise = 4.5 or noon - degrees(
                arccos(-tan(LatRad) * tan(SolarDeclination))
            ) / 15.
            SunSet  = 19.5 or noon - degrees(
                arccos(-tan(LatRad) * tan(SolarDeclination))
            ) / 15.
        """
        ...
        if 'Latitude_Radians' not in world:
            ...
            LatRad = world.setdefault('Latitude_Radians', np.radians(LatDeg))
        elif 'Latitude_Degrees' not in world:
            LatRad = world.setdefault('Latitude_Radians', np.radians(45.))

        if 'Longitude_Radians' not in world:
            LonDeg = world.pop('Longitude_Degrees', 0.)
            LonRad = world.setdefault('Longitude_Radians', np.radians(LonDeg))
        elif 'Longitude_Degrees' not in world:
            LonRad = world.setdefault('Longitude_Radians', np.radians(0.))

        world.setdefault('P', 101325.0)
        if (
            (self.usetheta or self.usesun)
            and 'StartDate' not in world
            and 'StartJday' not in world
        ):
            warn('WARN: Using SunRise and SunSet of 4.5 and 19.5 ('
                 + 'approximately JulianDay 145 and Latitude 45 degrees N)')
            ...
            SolarDeclination = solar_declination(
                StartJday
                + ((world['TSTART'] + world['TEND']) / 2. / 3600.) // 24
            )
            half_day = np.degrees(
                np.arccos(-np.tan(LatRad) * np.tan(SolarDeclination))
            ) / 15.
    ...
    def summary(self, verbose=0):
        ...
        -------
        sumtxt : str
            Text summary of mechanism describing
        """
        if verbose > 0:
        ...
        return (
            'Species: %d %s\nReactions: %d%s%s%s'
            % (len(self.allspcs), spcstr, len(self.get_rxn_strs()), rxnstr,
               initstr, initvaluesstr)
        )
    ...
    def get_rxn_strs(self, fmt=None):
        ...
        ---------
        fmt : str
            Format to use for stoiciometry in reactions.
        ...
        return [
            ' + '.join([
                '*'.join(
                    [spc] if fmt is None else [format(eval(stc), fmt), spc]
                )
                for stc, spc in rxn['reactants']
            ]) + ' = ' + ' + '.join([
                '*'.join(
                    [spc] if fmt is None else [format(eval(stc), fmt), spc]
                )
                for stc, spc in rxn['products']
            ]) + ': ' + rxn['rate'] + ';'
            for rxn in self._parsed['EQUATIONS']
        ]
    ...
    def print_rxns(self, fmt=None):
        ...
        -------
        rxnstr : str
            String describing all reactions
        """
        print('\n'.join(self.get_rxn_strs(fmt=fmt)))

    def print_monitor(self, y=None, t=None):
        """
        Print status of the mechanism state

        Arguments
        ---------
        y : array
            Array of mechanism state N=len(allspcs) in allspcs order
        t : float
    ...
    def output(self, outpath=None):
        ...
        ---------
        outpath : str
            Path to save archive results to.
    ...
    def get_output(self):
        ...
        outdf : pandas.DataFrame
            DataFrame read from self._archive
        """
        import pandas as pd
        self._archive.seek(0, 0)
    ...
    def archive(self):
        """
        Write state to archive file
        
        Returns
        -------
        outfile : file
            Arhcived results as a file-like object.
        """
        lookat = self.lookat
        if 't' not in lookat:
            lookat = ('t',) + lookat
        if 'P' not in lookat and 'P' in self.world:
            lookat = ('P',) + lookat
        if 'TEMP' not in lookat and 'TEMP' in self.world:
            lookat = ('TEMP',) + lookat
        if 'CFACTOR' not in lookat and 'CFACTOR' in self.world:
    ...
    def reorder(self):
        ...
        neworder = np.concatenate([
            densityasc[::-1][::2][::-1], densityasc[::-1][1::2]
        ])
        ...
        if self.banded:
            ...
            for di, d in enumerate(range(-self.ml, self.mu + 1)):
                if d < 0:
                    ...
                    before = []
                diagonals.append(
                    np.append(np.append(before, np.diagonal(out, d)),
                              after)[None]
                )
    ...
    def get_y(self, world=None):
        ...
        ---------
        world : dict
            Special dictionary to create vector (defaults to self.world)

        Returns
        -------
        y : array
            Evaluating each spc in all species in the context of world.
        """
        if world is None:
            world = self.world
        return np.array([eval(spc, None, world) for spc in self.allspcs])
    ...
    def update_y_from_y(self, y):
        ...
        -------
        y : array
            Update the y in world and return y
        """
        if 'y' in self.world:
            self.world['y'][:] = y
        else:
    ...
    def update_world_from_y(self, y=None):
        ...
        ---------
        y : array
            n=len(mech.allspcs) array of current state
    ...
    def set_spcs(self, **spcs):
        ...
        ---------
        spcs : mappable
            Values will be added to world and world's y parameter will be updated.

        Returns
        -------
        None
        """
        self.world.update(**spcs)
    ...
    def integrate(
        ...
        **solver_keywords
        ...
        ---------
        t0 : float
            starting time in (unit of kinetic rates)
        t1 : float
            ending time in (unit of kinetic rates)
        y0 : array
            vector of concentrations (unit of kinetic rates) in order of
            mech.allspcs
        solver : str
            String indicating solver approach (default None)
            - None if not provided, defaults to integrator set in
              mechanism defintion
            - 'odeint' use odeint with lsoda from ODEPACK
              (scipy.integrate.odeint)
            - 'lsoda', 'vode', 'zvode', 'dopri5', 'dopri853' use named
               solver with ode object (see scipy.integrate.ode)
        jac : bool
            (optional; default True) Use jacobian to speed up solution
        verbose : int
            (optional; default False) add additional printing
        solver_keywords : mappable
            (optional) solver keywords are specific to each solver; see
            scipy.integrate.ode for more details on keywords
            - with odeint, defaults to dict(atol=1e-3, rtol=1e-4,
              maxstep=1000, hmax=self.world['DT'], mxords=2, mxordn=2)
        ...
        if solver not in solvers:
            warn(
                'Solver %s may not exist; if you get an error try one of (%s)'
                % (solver, ', '.join(solvers))
            )
        ...
        if solver == 'odeint':
            default_solver_params = dict(
                atol=1e-3, rtol=1e-4, maxstep=1000, hmax=self.world['DT'],
                mxords=2, mxordn=2
            )
            ...
            if solver_keywords.get('col_deriv', False):
                self.drate_exp = compile(
                    self.drate_exp_str + '.T', 'drate_exp', 'eval'
                )

            # With full details
            if len(self.constraints) > 0:
                warnstr = (
                    "Constraints affect rates, but are not seen in outputs "
                    + "with odeint"
                )
                warn(warnstr)
            ...
            try:
                ...
                dy = self.dy
                ddy = self.ddy
                Ystep, infodict = itg.odeint(
                    dy, y0, ts, Dfun=(ddy if jac else None), full_output=True,
                    **solver_keywords
                )
                y0 = Ystep[-1]
                Y = np.vstack([Y, y0])
                ...
                errstr = (
                    str(e) + '\n\n ------------------------------- \n'
                    + ' If running again, you must reset the world '
                    + '(mech.resetworld())'
                )
            ...
            if solver_keywords.get('col_deriv', False):
                self.drate_exp = compile(
                    self.drate_exp_str, 'drate_exp', 'eval')
            ...
            while r.t < nextt:
                ...
                if (self.verbose + verbose) > 0 and not r.successful():
                    warn(
                        'Partial step from %.0f to %.0f instead of %.0f'
                        % (t0, r.t, nextt)
                    )
    ...
    def run(
        self, solver=None, tstart=None, tend=None, dt=None, jac=True,
        verbose=0, debug=False, **solver_keywords
    ):
        """
        ...
        tstart : float
            starting time in unit of kinetic rates (optional; default
            TSTART in initial values)
        tend : flaot
            ending time in unit of kinetic rates (optional; default
            TEND in initial values)
        dt : int
            time step in unit of kinetic rates (optional; default DT in
            initial values)
        jac : bool
            Use jacobian to speed up solution (default: True)
        verbose : int
            add additional printing (optional; default 0)
            - 0 no additional printing
            - 1 printing in run command and in first integration call
            - 2 or above add rpinting in run command and in all integration
              calls
        debug : bool
            set to true to enable pdb step-by-step execution
        solver_keywords : mappable
            solver keywords are specific to each solver; see
            scipy.integrate.ode for more details on keywords
            with odeint, defaults to dict(atol=1e-3, rtol=1e-4,
                maxstep=1000, hmax=self.world['DT'], mxords=2, mxordn=2)
            with ode, no defaults are set
        ...
        Notes
        ------
        Outputs mechname + '.tsv' - file with simulation
        """
        ...
        if tstart is None:
            tstart = self.world.get('TSTART', 12 * 3600)
        if tend is None:
            tend = self.world.get('TEND', (12 + 24) * 3600)
        if dt is None:
            dt = self.world.get('DT', 3600)
    ...
    def add_constraint(self, func):
        ...
        ---------
        func : function
            a function that takes a mechanism object and a world dictionary.
            The function should edit the species in world and in the world y
            vector to ensure
        ...
        def func_example_nox_constraint(mech, world):
            ...
            y = world['y']
            ind_NO, ind_NO2, ind_HO2NO2, ind_NO3, ind_N2O5 = eval(
    ...
    def get_rates(self, **kwds):
        """
        function that takes a mechanism object and a world dictionary.
        The function should edit the species in world and in the world y
        vector to ensure

        Steps:
            Update world from by adding these keywords.
            Update world in the context.
            Update y from that world.
            Update rate_const in that world.
            Evaluate rates

        Arguments
        ---------
        kwds : mappable
            Keywords to add to self.world

        Returns
        -------
        rxns_c_r : zip
            Return rxn, rate_const, and rates
        """
    ...
    def print_rates(self, **kwds):
        """
        Prints k, rate (k*pord(rcts)), and label

        Arguments
        ---------
        kwds : mappable
            Used as kwds in get_rates
    ...
    def print_world(self, out_keys=None, fmt='%.8e', verbose=0):
        """
        ...
        ---------
        out_keys : list
            List of elements to print. Defaults to t and monitor_expr
        fmt : str
        ...
        if (verbose + self.verbose) > 0:
            for ti in np.arange(t.size):
                ...
                for k in out_keys:
                    print(
                        ('%s=' + fmt)
                        % (k, self.world[k][ti] / (1 if k == 't' else cfactor))
                    )
                print()
        else:
            print(','.join(out_keys))
            for ti in np.arange(t.size):
                print(','.join([fmt % self.world[k][ti] for k in out_keys]))
