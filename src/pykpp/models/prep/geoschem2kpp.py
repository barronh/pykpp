...
geos_spec_pattern = r'^(?P<active>[AD])\s+(?P<ORD>\d+)\s+(?P<AR0>' + scinot + r')\s+(?P<BR0>' + scinot + r')\s+(?P<CR0>' + scinot + r') [012] (?P<type>(?:[ A-Z]|HR|QB)?)\s+(?P<FC>' + scinot + r')\s+(?P<FCT1>' + scinot + r')\.\s+(?P<FCT2>' + scinot + r')\.[ ]*h?(?P<BRKEY>\d)?[ ]*(\n\s+(?P<AR1>' + scinot + r')\s+(?P<BR1>' + scinot + r')\s+(?P<CR1>' + scinot + r')\s+0( [A-Z])?\s+0\.00\s+0\.\s+0\.\s+(\n\s+(?P<AR2>' + scinot + r')\s+(?P<BR2>' + scinot + r')\s+(?P<CR2>' + scinot + r')\s+0( [A-Z])?\s+0\.00\s+0\.\s+0\.\s+)?)?(?=\n )'
...
geos_std_reorder_pattern = r'(?P<key>{\s*(-)?\d+})[ ]+(?P<rate>(0\.00E\+00|TUV_J\(' + scinot + ', THETA\)|GEOS_Q\(TUV_J\(' + scinot + ', THETA\)\)|GEOS_[A-Z]{1,3}\((' + scinot + ', ){2,11}' + scinot + '\)))[ ]*\n\s+(?P<rxn>[^\n]+)\n'
...
template_dict = {' ': '{%(ORD)4d} GEOS_STD(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
                 ...
                 'F': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
                 'G': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e, %(AR1).6e, %(BR1).6e, %(CR1).6e)',
                 'HR': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e, %(AR1).6e, %(BR1).6e, %(CR1).6e)',
                 'K': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
                 'L': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
                 'N': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
                 'O': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
                 'P': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e, %(AR1).6e, %(BR1).6e, %(CR1).6e, %(FC).6e, %(FCT1).6e, %(FCT2).6e)',
                 'Q': '{%(ORD)4d} GEOS_%(type)s(TUV_J(%(AR0).6e, THETA))',
                 'T': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
                 'V': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e, %(AR1).6e, %(BR1).6e, %(CR1).6e)',
                 'W': '{%(ORD)4d} GEOS_%(type)s(%(AR0).6e, %(BR0).6e, %(CR0).6e)',
