...
Environment:
    The uninitialized environment includes a default temperature (298. K),
    pressure (101325 Pa), M, O2, N2, and H2.

Rate Evaluation:
    Because many models use different forms (e.g., signs), several models
    native forms are provided:

    GEOS-Chem associates different forms with a letter.  The basic form is
    provided by GEOS_STD, and then other forms are provided by GEOS_\S+ where
    \S+ is one or more non-white space (e.g., GEOS_A or GEOS_B).

    MOZART4 uses normal mathematical expressions and several special functions.
    Special functions include a troe form (MZ4_TROE) and numbered user
    expressions (MZ4_USR\d+) where \d+ is one or more digits.

    CMAQ uses special characters to key to numbered forms documented in the
    Science documentation appendix. CMAQ_1to4 is the most basic where all 4
    forms can be represented by supplying a 0 for one or more parameters.
    CMAQ_\d+ where \d+ is 5 to 10 are special functions (10 is the troe fall
    off equation).

    CHIMERE uses basic expressions and two special functions. Special functions
    are two forms of the TROE equation (CHIMERE_TROE and CHIMERE_MTROE).
...
def update_func_world(mech, world):
    ...
    for modkey in dir(funcs):
        ...
        if not hasattr(mod, 'update_func_world'):
            continue
...
def solar_noon_utc(LonDegE):
    ...
    _timezone = np.array([
        ...
        -52.5, -37.5, -22.5, -7.5, 7.5, 22.5, 37.5, 52.5, 67.5, 82.5, 97.5,
        112.5, 127.5, 142.5, 157.5, 172.5, 180
    ]).repeat(2, 0)[1:-1].reshape(-1, 2)
