__all__ = ['HET', 'PHOTOL', 'GCARR', 'GCIUPAC3', 'GCJPL3', 'GCJPLEQ', 'GCJPLPR', 'GC_DMSOH', 'GC_GLYCOHA', 'GC_GLYCOHB', 'GC_GLYXNO3', 'GC_HACOHA', 'GC_HACOHB', 'GC_HO2NO3', 'GC_OHCO', 'GC_OHHNO3', 'GC_RO2HO2', 'GC_RO2NO', 'GC_TBRANCH', 'HO2_H2O', 'OH_O1D', 'update_func_world']
...
def PHOTOL(pidx):
    ...
    gc2tuv = {
        ...
        61: "TUV_J(19, THETA)+TUV_J(21, THETA)", # ALD2 + hv = 0.880MO2 + HO2 + 0.880CO + 0.120MCO3
        62: "TUV_J(20, THETA)", # ALD2 + hv = CH4 + CO
        59: "TUV_J(40, THETA)+TUV_J(41, THETA)", # PAN + hv = 0.700MCO3 + 0.700NO2 + 0.300MO2 + 0.300NO3
        ...
        69: "TUV_J(28, THETA)", # MEK + hv = 0.850MCO3 + 0.850ETO2 + 0.150MO2 + 0.150RCO3
        68: "TUV_J(23, THETA)", # GLYC + hv = 0.900CH2O + 1.730HO2 + CO + 0.070OH + 0.100MOH
        ...
        86: "TUV_J(31, THETA)", # RIP + hv = OH + HO2 + 0.710CH2O + 0.425MVK +  0.285MACR + 0.290HC5
        87: "TUV_J(31, THETA)", # IAP + hv = OH + HO2 + 0.670CO + 0.190H2 +  0.360HAC + 0.260GLYC + 0.580MGLY
        88: "TUV_J(31, THETA)", # ISNP + hv = OH + HO2 + RCHO + NO2
        89: "TUV_J(31, THETA)", # VRP + hv = OH + 0.300HO2 + 0.300CH2O +  0.700MCO3 + 0.700GLYC + 0.300MGLY
        90: "TUV_J(31, THETA)", # MRP + hv = OH + HO2 + HAC + 0.500CO + 0.500CH2O
        91: "TUV_J(31, THETA)", # MAOP + hv = OH + CH2O + MCO3
        98: "TUV_J(36, THETA)", # chose iso-butane nitrate?R4N2 + hv = NO2 + 0.320ACET + 0.190MEK + 0.180MO2 + 0.270HO2 + 0.320ALD2 + 0.130RCHO + 0.050A3O2 + 0.180B3O2 + 0.320ETO2
        99: "TUV_J(33, THETA)", # MAP + hv = OH + MO2
        92: "TUV_J(37, THETA)", # probably not right index?MACRN + hv = NO2 + HAC + MGLY + 0.500CH2O + HO2 + 0.500CO
        93: "TUV_J(37, THETA)", # probably not right index?MVKN + hv = GLYC + NO2 + MCO3
        94: "TUV_J(37, THETA)", # probably not right index?ISOPNB + hv = HC5 + NO2 + HO2
...
def GC_OHHNO3( A0,B0,C0,A1,B1,C1,A2,B2,C2 ):
    ...
    R0 =  float(A0) * exp(float(C0)/TEMP) * (300./TEMP)**float(B0)
    R1 =  float(A1) * exp(float(C1)/TEMP) * (300./TEMP)**float(B1)
    R2 =  NUMDEN*(float(A2) * exp(float(C2)/TEMP) * (300./TEMP)**float(B2))
...
def GC_GLYCOHA( A0,B0,C0 ):
    ...
    if (GLYC_FRAC<0e+0): GLYC_FRAC=0e+0
    return R0*GLYC_FRAC
...
def GC_HACOHA( A0,B0,C0 ):
    ...
    if (HAC_FRAC<0e+0): HAC_FRAC=0e+0
    return R0*HAC_FRAC
...
def GCJPL3( k0_300, n, ki_300, m):
    ...
    GCJPL3=(k0/(1.e0+k0/(ki/NUMDEN)))*0.6** \
           ((1+((log10(k0/(ki/NUMDEN)))**2e0)**1.0e0))
...
def GCJPLEQ( A0,B0,C0,A1,B1,C1,A2,B2,C2,FV,FCT1,FCT2 ):
    ...
    if (not A2 is None): # P-dependent
        if B2 is None or C2 is None or FV is None or FCT1 is None or FCT2 is None:
            print('GCJPLEQ: Missing parameters for P-dependent reaction.') # Missing params!
